...
__global__ void reverseArrayBlock(int *d_out, int *d_in)
{
    ...
    int index_in = blockDim.x * blockIdx.x + threadIdx.x;
    s_data[blockDim.x - 1 - threadIdx.x] = d_in[index_in];
    ...
    int index_out = blockDim.x * (gridDim.x - 1 - blockIdx.x) + threadIdx.x;
    d_out[index_out] = s_data[threadIdx.x];
}
